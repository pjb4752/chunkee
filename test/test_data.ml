...
let pervasive_root = C.Module.Name.from_string "test"
...
let pervasive_module =
  let name = C.Module.Name.from_string "pervasive" in
  let path = C.Module.Path.from_list [pervasive_root] in
  let mempty = C.Module.from_parts path name in
  List.fold_left (fun m (n, t) ->
    let n = C.Module.Var.Name.from_string n in
