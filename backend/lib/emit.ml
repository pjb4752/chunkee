...
let emit_number value =
  Lua_snippet.make_expression (sprintf "%f" value)

let emit_string value =
  Lua_snippet.make_expression (sprintf "\"%s\"" value)
...
let emit_symbol ?wrap_ops:(wrap_ops=true) value =
  Lua_snippet.make_expression @@ emit_name ~wrap_ops:wrap_ops value

let emit_def recursively_emit name_generator name expression =
  let name = Identifier.to_string name |> escape_name in
  let expression_snippet = recursively_emit name_generator expression in
  let result_expression = Lua_snippet.result_expression expression_snippet in
  let assignment = sprintf "%s = %s" name result_expression in
  let def_statement = Lua_snippet.make_unit_statement assignment in
  Lua_snippet.insert_preamble def_statement expression_snippet
...
let emit_function recursively_emit name_generator parameters body =
  ...
  let body_expression = Lua_snippet.lua_string body_expression in
  let function_expression = sprintf "function(%s)\n%s\nend" parameters body_expression in
  Lua_snippet.make_expression function_expression
...
let emit_if recursively_emit name_generator test_expression if_expression else_expression =
  ...
  let test_result = Lua_snippet.result_expression test_expression in
  let if_snippet = recursively_emit name_generator if_expression in
  let if_string = Lua_snippet.lua_string ~target:expression_target if_snippet in
  let else_snippet = recursively_emit name_generator else_expression in
  let else_string = Lua_snippet.lua_string ~target:expression_target else_snippet in
  let combined_string = String.concat "\n" [
    sprintf "%s nil\nif %s then" expression_target test_result;
    sprintf "%s\nelse\n%s\nend" if_string else_string
  ] in
  let if_statement = Lua_snippet.make_result_statement generated_name combined_string in
  Lua_snippet.insert_preamble if_statement test_expression
...
let build_binding_statements recursively_emit bindings =
  List.map (fun binding ->
    ...
    Lua_snippet.lua_string ~target:result (recursively_emit expression)) bindings
...
let emit_let recursively_emit name_generator bindings body =
  ...
  let body_snippet = recursively_emit name_generator body in
  let body_string = Lua_snippet.lua_string ~target:expression_target body_snippet in
  let combined_string = sprintf "%s nil\ndo\n%s\n%s\nend" expression_target binding_string body_string in
  Lua_snippet.make_result_statement generated_name combined_string
...
let build_arguments recursively_emit arguments =
  ...
  let result_expressions = List.map Lua_snippet.result_expression argument_expressions in
  (result_expressions, argument_expressions)

let emit_infix_apply recursively_emit operator arguments =
  let operator = Lua_snippet.result_expression (emit_symbol ~wrap_ops:false operator) in
  ...
  let application = Lua_snippet.make_expression application in
  List.fold_left Lua_snippet.insert_preamble application argument_expressions

let emit_prefix_apply recursively_emit function_name arguments =
  let function_name = Lua_snippet.result_expression (emit_symbol function_name) in
  let (result_expressions, argument_expressions) = build_arguments recursively_emit arguments in
...
let emit_literal_apply recursively_emit name_generator function_literal arguments =
  let (name_generator, generated_name) = Name_gen.next_value name_generator in
  let function_expression = Lua_snippet.result_expression (recursively_emit name_generator function_literal) in
  let function_assignment = sprintf "%s = %s" generated_name function_expression in
  let function_preamble = Lua_snippet.make_result_statement generated_name function_assignment in
...
let emit_get record field =
  ...
  | Node.Symbol (name, _) -> begin
    ...
    Lua_snippet.make_expression (sprintf "%s.%s" record field)
  end
  | _ -> assert false
...
let emit_set recursively_emit name_generator record field expression =
  ...
  | Node.Symbol (name, _) -> begin
    ...
    let assignment = Lua_snippet.make_unit_statement assignment in
    Lua_snippet.insert_preamble assignment expression_snippet
  end
  | _ -> assert false
...
let rec emit_node name_generator node =
  ...
  | Node.Rec _ -> Lua_snippet.make_expression ""
