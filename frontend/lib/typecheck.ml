...
let typecheck_test_form recursively_typecheck scopes position test_form =
  ...
  return (check_test_type position test_type)
...
let typecheck_if recursively_typecheck scopes position test_form if_form else_form =
  ...
  let* return_type = typecheck_if_branches position if_type else_type in
  return return_type
...
let typecheck_parameter_types position expected_types actual_types return_type =
...
let typecheck_get symbol_symbol_table scopes position target_form field =
  ...
  | { Form.parsed = Form.Symbol name; _ } -> begin
    ...
    let* target_fields = typecheck_record_type position target_type in
    let* target_type = typecheck_record_field position target_fields field in
    return target_type
  end
  | _ -> assert false
...
let typecheck_set recursively_typecheck symbol_symbol_table scopes position target_form field body_form =
  ...
  | { Form.parsed = Form.Symbol name; _ } -> begin
    ...
    let* _ = compare_set_type position target_type actual_type in
    return Type.Unit
  end
  | _ -> assert false
